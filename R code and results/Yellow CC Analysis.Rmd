...
```{r, echo=TRUE}
test1<-read.csv(file="/Users/guoxinyu/Documents/USC/Final Thesis/stage 2/testY CC.csv",header = TRUE,sep =",")
...
#pngRes <- getAmigoTree(goIDs=goIDs, pvalues=pvalues, pcolors = c("white","tomato"),psplit = c(1,0.25,0.1,0.05,0.001), filename="/Yellow CC Fisher", picType="png", saveResult=TRUE)

#pvalues1<-rep(0.0001, length(xlsxBP$ID))
#pngRes1<- getAmigoTree(goIDs=xlsxBP$ID, pvalues=pvalues1, pcolors = c("white","tomato"),psplit = c(1,0.25,0.1,0.05,0.001), filename="/Paper Yellow CC Amigo", picType="png", saveResult=TRUE)
```
