...
```{r, echo=TRUE}
test1<-read.csv(file="/Users/guoxinyu/Documents/USC/Final Thesis/stage 2/testY BP.csv", header=TRUE, sep=",")
...
xlsxBP<-xlsx[xlsx$Ontology=="BP",c(1,3,6,7,5)]
...
#pngRes <- getAmigoTree(goIDs=goIDs, pvalues=pvalues, pcolors = c("white","tomato"),psplit = c(1,0.25,0.1,0.05,0.001), filename="/Yellow BP Amigo", picType="png", saveResult=TRUE)

#pvalues1<-rep(0.0001, length(xlsxBP$ID))
#pngRes1<- getAmigoTree(goIDs=xlsxBP$ID, pvalues=pvalues1, pcolors = c("white","tomato"),psplit = c(1,0.25,0.1,0.05,0.001), filename="/Yellow BP author", picType="png", saveResult=TRUE)
```
